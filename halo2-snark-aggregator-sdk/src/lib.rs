        ...
        mod zkcli {
            ...
            impl CliBuilder {
                ...
                fn dispatch_verify_setup(&self) {
                    ...
                    // Why write params again when they are already cached somewhere else?
                    // Just symlink the params file to "output/verify_circuit.params" manually
                ...
                fn dispatch_verify_run(&self) {
                    ...
                    let verify_circuit_params = &halo2_snark_aggregator_circuit::fs::get_params_cached::<G1Affine, Bn256>(self.verify_circuit_k);
                    ...
                    let request = MultiCircuitsCreateProof::<_, _, $n> {
                        ...
                        // verify_circuit_vk: load_verify_circuit_vk_cached_params(&mut self.folder.clone(), verify_circuit_params),
                        coherent: $coherent,
                        verify_public_inputs_size: self.compute_verify_public_input_size()
                    };

                    let (pk, final_pair, instance, proof) = request.call();
                    ...
                    // might as well do the solidity stuff here too since vk write doesn't work
                    ...
                    let request = MultiCircuitSolidityGenerate::<G1Affine, $n> {
                        ...
                        verify_vk: pk.get_vk(),
                        verify_circuit_instance: vec![vec![instance]],
                        proof,
                        verify_public_inputs_size: self.compute_verify_public_input_size(),
                    };
