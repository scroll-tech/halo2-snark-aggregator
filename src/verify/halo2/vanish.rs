...
impl<'a, C, S:Clone, P:Clone, Error:Debug> Evaluated<'a, C, S, P, Error> {
    pub(in crate::verify::halo2) fn new(
        ...
        random_eval: &'a S,
        expect_commitments: Vec<&'a P>) -> Evaluated<'a, C, S, P, Error> {
        ...
        let h_commitment = expect_commitments.iter().rev()
            .fold(scalar!(zero), |acc, commitment| {
                let c = CommitQuery {c: Some(commitment.clone()), v:None};
    ...
    pub(in crate::verify::halo2) fn queries(
        ...
        iter::empty()
        .chain(
            Some(EvaluationQuery::new_from_query(
            x.clone(),
